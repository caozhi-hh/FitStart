...
function renderMarkdown(content: string): ReactNode[] {
  ...
  lines.forEach((line, index) => {
    ...
    if (/^\d+\.\s/.test(line)) {
      ...
      if (index === lines.length - 1 || !/^\d+\.\s/.test(lines[index + 1])) {
        elements.push(
          <ol key={`olist-${index}`} className="list-decimal list-inside space-y-1 my-3 text-gray-700">
            {listItems.map((item, i) => (
              <li key={i}>{item}</li>
            ))}
          </ol>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="font-semibold text-gray-900">
              {article.user?.username || '健身达人'}
            </div>
            <div className="text-sm text-gray-500">健身经验分享者</div>
          </div>
        </div>
      </div>
            ...
            .map((relatedArticle) => (
              ...
              >
                ...
                <div className="text-sm text-gray-500 mt-1">
                  {relatedArticle.views} 阅读 · {relatedArticle.likes} 点赞
                </div>
              </Link>
            ))}
        </div>
